...
\begin{frame}{PCA - Goal}
  \begin{columns}[T]
    ...
    \begin{block}{Objective}
        \centering\normalsize
        Find the orthonormal projection matrix $V_k \in \mathbb{R}^{d \times k}$ that \textbf{maximises the variance} of the projected data:
        \[
        \max_{V_k^\top V_k = I}\; \operatorname{tr}\!\left(V_k^\top \Sigma\, V_k\right)
        \]
    \end{block}
    \end{column}
  ...
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
      \begin{alertblock}{ Key Limitations}
        \begin{itemize}\small
          \item \textbf{Linearity} PCA can only capture linear relationships, fails on nonlinear manifolds.
...
\begin{frame}{t-SNE - Goal}
  ...
  \begin{block}{Objective}
    \centering\normalsize
    Find a low-dimensional embedding $Y$ that \textbf{minimises the KL divergence} between the high-dimensional pairwise similarity distribution $P$ and its low-dimensional counterpart $Q$.
  \end{block}

    \begin{figure}
        \centering
        \includegraphics[width=0.65\textwidth]{tsne.jpg}
        \caption{t-SNE: preserving local similarities by matching probability distributions.}
    \end{figure}
...
\begin{frame}{t-SNE - Disadvantages}
    \begin{alertblock}{ Key Limitations}
    \begin{itemize}\small
        \item \textbf{Scalability}: $O(n^2)$ memory/time.
...
\begin{frame}{UMAP - Goal}
  ...
  \begin{block}{Objective}
    \centering\normalsize
    Use gradient descent to find a low-dimensional embedding that has samilar topological structure to the high-dimensional data, by minimising the cross-entropy.
  \end{block}
    \end{column}
    \begin{column}{0.52\textwidth}
      \centering
    \begin{figure}
        \centering
        \includegraphics[width=0.65\textwidth]{UMAP.png}
        \caption{UMAP: preserving topological structure in the manifold space.}
    \end{figure}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{K-means - Goal}
  ...
  \begin{block}{Objective}
    \centering\normalsize
    Find cluster assignments $\{C_j\}$ and centroids $\{\mu_j\}$ that \textbf{minimise the total within-cluster sum of squared distances}:
    \[
      \min_{\{C_j\},\{\mu_j\}}\;\sum_{j=1}^{k}\sum_{x_i \in C_j} \|x_i - \mu_j\|^2
    \]
  \end{block}
    \end{column}
    \begin{column}{0.52\textwidth}
...
\begin{frame}{DBSCAN - Goal}
  ...
  \begin{block}{Objective}
    \centering\normalsize
    Partition points into clusters of \textbf{arbitrary shape} by grouping all points that are \emph{density-reachable} from at least one core point (i.e.\ have $\geq \mathit{minPts}$ neighbours within radius $\varepsilon$), while labelling all remaining points as \textbf{noise}:
    \[
      C_j = \bigl\{x_i \mid x_i \text{ is density-reachable from some core point } x_c \in C_j\bigr\}
    \]
  \end{block}
    \end{column}
    \begin{column}{0.52\textwidth}
...
\begin{frame}{HDBSCAN - Goal}
  ...
  \begin{block}{Objective}
    \centering\normalsize
    Extract the flat clustering that \textbf{maximises the total cluster stability} across the full density hierarchy, where stability measures how persistently a cluster survives as the density threshold $\lambda$ is raised:
    \[
      \max_{\text{flat clustering}}\;\sum_{C \in \text{selected clusters}} \mathrm{stability}(C),
      \quad \mathrm{stability}(C) = \sum_{x_i \in C}\!\bigl(\lambda_i^{\mathrm{death}} - \lambda_i^{\mathrm{birth}}\bigr)
    \]
  \end{block}
    \end{column}
    \begin{column}{0.52\textwidth}
